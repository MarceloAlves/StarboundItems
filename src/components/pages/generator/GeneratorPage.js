...
class GeneratorPage extends Component {
  constructor(props) {
    ...
    this.handleCommandChange = this
      .handleCommandChange
      .bind(this)
    this.handleItemChange = this
      .handleItemChange
      .bind(this)
    this.handleJsonChange = this
      .handleJsonChange
      .bind(this)
  }
  componentDidMount() {
  ...
  searchOptions(input, callback) {
    const results = performSearch(input, true);
  ...
  handleItemChange(e) {
    this.setState({selectedItem: e.value, jsonDiff: [], itemJson: e.originalJson, originalJson: e.originalJson})
  }
        ...
        <div className="columns">
          <div className="column">
            <p>
              ...
              <code>/spawnitem</code>
              is available), then search for an item and select one.</p>
            <p>A text box with the original JSON will appear on the left. The right will be
              a text box with the command to spawn that item. Changes made to the JSON will
              update the command. Hopefully it works.</p>
            <p>It's under development and there could be issues. If you run into issues or
              have some feedback, please use this
